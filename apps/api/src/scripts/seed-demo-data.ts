...
import { organizations, users, pipelines, pipelineStages, accounts, contacts, deals, activities, activityChunks } from '../db/schema';
...
async function main() {
  ...
  if (!org) throw new Error('No organization found — run the app and sign up a user first.');
  ...
  const [pipeline] = await db.select().from(pipelines).where(eq(pipelines.organizationId, org.id)).limit(1);
  ...
  console.log(`Seeding into org "${org.name}" (${org.id}), pipeline "${pipeline.name}"\n`);

  for (const demo of demoDeals) {
    const stageId = stageByName.get(demo.stageName);
    if (!stageId) {
        console.warn(`Skipping ${demo.companyName} — stage "${demo.stageName}" not found`);
        continue;
    }
    ...
    const [account] = await db.insert(accounts).values({
        organizationId: org.id, ownerId: owner.id, name: demo.companyName, domain: demo.domain,
    }).returning();

    const [contact] = await db.insert(contacts).values({
        organizationId: org.id, ownerId: owner.id, accountId: account.id,
        firstName: demo.contactFirstName, lastName: demo.contactLastName, email: demo.contactEmail,
    }).returning();

    const [deal] = await db.insert(deals).values({
        organizationId: org.id, ownerId: owner.id, accountId: account.id, contactId: contact.id,
        pipelineId: pipeline.id, stageId, title: demo.dealTitle, value: demo.value.toString(), status: demo.status,
    }).returning();

    // 1. Create all activity rows first (no embeddings yet), tracking each
    //    note's chunk texts alongside its activityId.
    const chunkJobs: { activityId: string; chunkIndex: number; body: string }[] = [];
    for (const noteBody of demo.notes) {
        const [activity] = await db.insert(activities).values({
        organizationId: org.id, entityType: 'deal', entityId: deal.id, type: 'note', body: noteBody, createdBy: owner.id,
        }).returning();
        const chunks = chunkText(noteBody);
        chunks.forEach((chunkBody, i) => chunkJobs.push({ activityId: activity.id, chunkIndex: i, body: chunkBody }));
    }

    // 2. One batched Voyage call for this deal's entire note set.
    const response = await voyage.embed({ input: chunkJobs.map((c) => c.body), model: 'voyage-4-lite' });
    ...
    for (let i = 0; i < chunkJobs.length; i++) {
        ...
        if (!embedding) throw new Error(`No embedding at index ${i} for ${demo.companyName}`);
        await db.insert(activityChunks).values({
        activityId: chunkJobs[i].activityId, organizationId: org.id, entityType: 'deal', entityId: deal.id,
        chunkIndex: chunkJobs[i].chunkIndex, body: chunkJobs[i].body, embedding,
        });
    }

    console.log(`  -> ${demo.notes.length} notes, ${chunkJobs.length} chunk(s) embedded in 1 API call`);
...
const demoDeals: DemoDeal[] = [
  {
    companyName: 'Nordwind Logistics', domain: 'nordwind-logistics.com',
    contactFirstName: 'Erik', contactLastName: 'Larsson', contactEmail: 'erik.larsson@nordwind-logistics.com',
    dealTitle: 'Nordwind Logistics - Fleet Tracking Deal', value: 12000, stageName: 'Qualified', status: 'open',
    notes: [
  ...
  {
    companyName: 'Bramble & Co', domain: 'brambleco.com',
    contactFirstName: 'Priya', contactLastName: 'Nair', contactEmail: 'priya.nair@brambleco.com',
    dealTitle: 'Bramble & Co - Retail Analytics Suite', value: 8500, stageName: 'New', status: 'open',
    notes: [
  ...
  {
    companyName: 'Vantage Point Consulting', domain: 'vantagepointconsulting.io',
    contactFirstName: 'Marcus', contactLastName: 'Webb', contactEmail: 'marcus.webb@vantagepointconsulting.io',
    dealTitle: 'Vantage Point - Client Reporting Platform', value: 22000, stageName: 'Qualified', status: 'won',
    notes: [
  ...
  {
    companyName: 'Ferro Manufacturing', domain: 'ferromfg.com',
    contactFirstName: 'Diane', contactLastName: 'Castellano', contactEmail: 'diane.castellano@ferromfg.com',
    dealTitle: 'Ferro Manufacturing - Inventory Forecasting', value: 15000, stageName: 'New', status: 'lost',
    notes: [
  ...
  {
    companyName: 'Solstice Wellness Group', domain: 'solsticewellness.com',
    contactFirstName: 'Amara', contactLastName: 'Okafor', contactEmail: 'amara.okafor@solsticewellness.com',
    dealTitle: 'Solstice Wellness - Member Management System', value: 6000, stageName: 'Qualified', status: 'open',
    notes: [
  ...
  {
    companyName: 'Ironclad Legal Partners', domain: 'ironcladlegal.com',
    contactFirstName: 'Robert', contactLastName: 'Chen', contactEmail: 'robert.chen@ironcladlegal.com',
    dealTitle: 'Ironclad Legal - Document Automation', value: 18000, stageName: 'New', status: 'open',
    notes: [
  ...
  {
    companyName: 'Coastal Brew Collective', domain: 'coastalbrewcollective.com',
    contactFirstName: 'Lena', contactLastName: 'Fitzgerald', contactEmail: 'lena.fitzgerald@coastalbrewcollective.com',
    dealTitle: 'Coastal Brew Collective - POS Integration', value: 4500, stageName: 'Qualified', status: 'won',
    notes: [
  ...
  {
    companyName: 'Meridian Health Partners', domain: 'meridianhealthpartners.org',
    contactFirstName: 'Dr. Sarah', contactLastName: 'Kim', contactEmail: 's.kim@meridianhealthpartners.org',
    dealTitle: 'Meridian Health - Patient Scheduling Overhaul', value: 30000, stageName: 'New', status: 'lost',
    notes: [
